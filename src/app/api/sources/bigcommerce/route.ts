...
import { testBigCommerceConnection } from "@/connectors/bigcommerce";
...
const bodySchema = z.object({
  ...
  storeHash: z
    ...
    .min(1)
    .max(32)
    .regex(/^[a-z0-9]+$/i, { message: "store hash looks invalid" }),
  storefrontUrl: z
    .string()
    .url()
...
/** Create a BigCommerce source: connection-test first, token encrypted. */
export async function POST(req: Request) {
  ...
  const { name, storeHash, storefrontUrl, accessToken } = parsed.data;
  const config = { storeHash, ...(storefrontUrl ? { storefrontUrl } : {}) };

  try {
    await testBigCommerceConnection(config, { accessToken });
  } catch (e) {
    return NextResponse.json(
