...
- `src/model/` — the canonical product model (ADR-0009): `CanonicalProduct`/`CanonicalVariant` zod schemas with ACP-spec constraints + `validateProduct()` returning `{field, code, severity, message}` issues (the audit_findings shape). The single contract between connectors and feeds/audit/enrichment. Seller-level ACP fields live on the merchant, attached at feed render.
